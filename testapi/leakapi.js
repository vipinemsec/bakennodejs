...
app.get('/leak', async (req, res) => {
    ...
    try {
        lld = await axios.post(API_URL, `key=${API_KEY}&type=domain&query=${DOMAIN}`, {
            headers: {
                // 'Content-Type': 'application/x-www-form-urlencoded'
                ' Accept':'express.application/json'
            }
        });
        res.send(lld.data)
    } catch (error) {
    ...
    // res.send(lld)
    console.log("working on leakapi")
    // const KEY="96f8eb26076919781f2ef87dfcb05a91"
